...
export default function Dashboard() {
  ...
  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto">
      ...
      {showPassageForm && (
        <div className="bg-white/70 border border-purple-200 p-6 rounded-2xl shadow-xl mb-6">
          <h3 className="text-lg font-bold text-purple-900 mb-4">إضافة نص قرائي وأسئلة جديدة</h3>
          <form onSubmit={handleAddPassage} className="flex flex-col gap-4">
            <textarea 
              value={newPassage} 
              onChange={(e) => setNewPassage(e.target.value)} 
              placeholder="اكتب النص هنا..." 
              className="p-3 rounded-xl bg-white border border-purple-100 focus:ring-2 focus:ring-purple-400 h-24 text-sm" 
              required 
            />
            
            <div className="border-t pt-4">
              <h4 className="font-bold text-purple-800 mb-3">السؤال الأول</h4>
              <input 
                value={q1} 
                onChange={(e) => setQ1(e.target.value)} 
                placeholder="نص السؤال الأول" 
                className="w-full p-2 mb-3 rounded-lg bg-white border border-purple-100 text-sm" 
                required 
              />
              <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                <input value={o1a} onChange={(e) => setO1a(e.target.value)} placeholder="الخيار الأول" className="p-2 rounded-lg bg-white border border-purple-100 text-sm" required />
                <input value={o1b} onChange={(e) => setO1b(e.target.value)} placeholder="الخيار الثاني" className="p-2 rounded-lg bg-white border border-purple-100 text-sm" required />
                <input value={o1c} onChange={(e) => setO1c(e.target.value)} placeholder="الخيار الثالث" className="p-2 rounded-lg bg-white border border-purple-100 text-sm" required />
                <input value={a1} onChange={(e) => setA1(e.target.value)} placeholder="الإجابة الصحيحة" className="p-2 rounded-lg bg-emerald-50 border border-emerald-200 text-sm" required />
              </div>
            </div>

            <div className="border-t pt-4">
              <h4 className="font-bold text-purple-800 mb-3">السؤال الثاني</h4>
              <input 
                value={q2} 
                onChange={(e) => setQ2(e.target.value)} 
                placeholder="نص السؤال الثاني" 
                className="w-full p-2 mb-3 rounded-lg bg-white border border-purple-100 text-sm" 
                required 
              />
              <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                <input value={o2a} onChange={(e) => setO2a(e.target.value)} placeholder="الخيار الأول" className="p-2 rounded-lg bg-white border border-purple-100 text-sm" required />
                <input value={o2b} onChange={(e) => setO2b(e.target.value)} placeholder="الخيار الثاني" className="p-2 rounded-lg bg-white border border-purple-100 text-sm" required />
                <input value={o2c} onChange={(e) => setO2c(e.target.value)} placeholder="الخيار الثالث" className="p-2 rounded-lg bg-white border border-purple-100 text-sm" required />
                <input value={a2} onChange={(e) => setA2(e.target.value)} placeholder="الإجابة الصحيحة" className="p-2 rounded-lg bg-emerald-50 border border-emerald-200 text-sm" required />
              </div>
            </div>

            <button type="submit" className="bg-purple-600 text-white px-6 py-2 rounded-xl font-bold w-fit text-sm mt-2 hover:bg-purple-700 transition">
              حفظ النص والأسئلة
            </button>
          </form>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-white/70 backdrop-blur-xl border border-purple-100 p-5 rounded-2xl shadow-md">
          <h3 className="text-xs font-bold text-purple-700 uppercase tracking-wide">إجمالي الجلسات</h3>
          <p className="text-3xl font-extrabold text-indigo-600 mt-2">{sessions.length}</p>
        </div>
        <div className="bg-white/70 backdrop-blur-xl border border-purple-100 p-5 rounded-2xl shadow-md">
          <h3 className="text-xs font-bold text-pink-700 uppercase tracking-wide">متوسط الدقة</h3>
          <p className="text-3xl font-extrabold text-pink-600 mt-2">
            {sessions.length > 0 ? (sessions.reduce((acc: number, s: any) => acc + s.accuracy_percent, 0) / sessions.length).toFixed(1) : 0}%
          </p>
        </div>
        <div className="bg-white/70 backdrop-blur-xl border border-purple-100 p-5 rounded-2xl shadow-md">
          <h3 className="text-xs font-bold text-cyan-700 uppercase tracking-wide">متوسط السرعة</h3>
          <p className="text-3xl font-extrabold text-cyan-600 mt-2">
            {sessions.length > 0 ? Math.round(sessions.reduce((acc: number, s: any) => acc + s.wpm, 0) / sessions.length) : 0} <span className="text-sm text-purple-400">WPM</span>
          </p>
        </div>
      </div>

      <div className="bg-white/70 backdrop-blur-xl border border-purple-100 p-5 rounded-2xl shadow-md mb-6">
        ...
        <div className="w-full h-64" dir="rtl">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
              ...
              <Tooltip contentStyle={{ background: 'rgba(255,255,255,0.9)', border: '1px solid #d8b4fe', borderRadius: '12px', fontSize: '12px' }} />
          ...
          <table className="min-w-full text-right border-collapse">
            <thead>
              <tr className="border-b border-purple-100">
                ...
                <th className="py-3 px-3 text-xs font-bold text-purple-700">النص المقروء (AI) والصوت</th>
              </tr>
            </thead>
            <tbody>
              {sessions.length === 0 ? (
                ...
                sessions.map((session: any) => (
                  <tr key={session.session_id} className="border-b border-purple-50 hover:bg-white/60 align-top">
                    <td className="py-3 px-3 font-bold text-purple-900 text-sm whitespace-nowrap">{session.student_username}</td>
                    <td className="py-3 px-3">
                      <span className={`px-2 py-1 rounded-full text-xs font-bold ${session.accuracy_percent > 85 ? 'bg-emerald-100 text-emerald-700' : session.accuracy_percent > 60 ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'}`}>
                        {session.accuracy_percent}%
                      </span>
                    </td>
                    <td className="py-3 px-3 text-purple-600 font-bold text-sm whitespace-nowrap">{session.wpm} <span className="text-xs text-purple-300">WPM</span></td>
                    ...
                    <td className="py-3 px-3 text-purple-500 max-w-[300px] text-xs leading-relaxed">
                      <div className="bg-white/60 rounded-lg p-2 border border-purple-50">
                        <p className="italic mb-2">"{session.asr_transcript}"</p>
                        {session.audio_file_id && (
                          <audio controls src={session.audio_file_id} className="w-full h-8 mt-1">Your browser does not support the audio element.</audio>
                        )}
                      </div>
                    </td>
                  </tr>
